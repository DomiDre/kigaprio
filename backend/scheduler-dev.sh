...
echo "Starting PrioTag cleanup scheduler (DEVELOPMENT MODE)..."
...
*/1 * * * * cd /app && /home/ubuntu/.local/bin/uv run python -m priotag.scripts.run_cleanup_tasks

# Run monitoring updates every 2 minutes (instead of 5) for testing
*/1 * * * * cd /app && /home/ubuntu/.local/bin/uv run python -m priotag.scripts.run_monitoring
EOF
...
echo "   docker compose exec scheduler uv run python -m priotag.scripts.run_cleanup_tasks"
echo ""
